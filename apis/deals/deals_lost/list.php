...
- from, to (date filter on lost_at: YYYY-MM-DD)
- page, limit
*/
...
/* ✅ q search (single search key) 
   q=ragul / q=10000 / q=reason / q=5 etc...
   searches in: title, lost_reason, stage, value, id, customer_id, original_deal_id
*/
if ($q !== '') {
    if (ctype_digit($q)) {
        $qi = (int)$q;
        $where[] = "(
            ...
            OR title LIKE ? OR lost_reason LIKE ? OR stage LIKE ?
        )";
        ...
        $where[] = "(
            title LIKE ? OR lost_reason LIKE ? OR stage LIKE ?
        )";
        $like = "%".$q."%";
        $params[] = $like; $params[] = $like; $params[] = $like;
        $types .= "sss";
...
/* ================= TOTAL ================= */
$countSql = "SELECT COUNT(*) AS total FROM deals_lost $whereSql";
